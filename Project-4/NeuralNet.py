...
class Perceptron(object):
    ...
    def getWeightedSum(self, inActs):
        ...
        Inputs:
            inActs (list<float/int>): input values, same length as inSize
        Returns:
            float
            The weighted sum
        """
    ...
    def sigmoid(self, value):
        ...
        """
        eulers_number = exp(1)
        return  1 / ( 1 + eulers_number**(-value))
    ...
    def sigmoidActivation(self, inActs):
        ...
        inActs = [1] + inActs # Add bias input to input vector
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        inActs = [1] + inActs  # Include the bias in the input activations

        for i in range(len(self.weights)):
            modification = alpha * delta * inActs[i]
            self.weights[i] += modification  # Update the weight
            totalModification += abs(modification)  # Add the absolute modification to the total
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                activation = perceptron.sigmoidActivation(output_list[-1])  # Use the output of the last layer
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in range(len(example[1])):
                previous_layer_output = allLayerOutput[-2] if self.numHiddenLayers > 0 else allLayerOutput[0]
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(previous_layer_output)
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = sum(nextLayer[neuronIndex].weights[neuronNum + 1] * deltas[0][neuronIndex] 
                            for neuronIndex in range(len(nextLayer)))
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold=0.00008, hiddenLayerList=[1], maxItr=sys.maxsize, startNNet=None):
    ...
    for example in examplesTest:
        predicted = nnet.feedForward(example[0])[-1]
        rounded = [round(act) for act in predicted]
    ...
    print('Feed Forward Test correctly classified %d, incorrectly classified %d, test accuracy %f'% (testCorrect, testError, testAccuracy))

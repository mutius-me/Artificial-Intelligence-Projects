...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i, position in enumerate(maxBeliefs):
            if livingGhostPositionDistributions[i][position] >= maxProbability:
                nearestGhostPosition = position
                maxProbability = livingGhostPositionDistributions[i][position]

        for a in legal:
            nextPacmanLocation = Actions.getSuccessor(pacmanPosition, a)
            nearestGhostDistance.append(self.distancer.getDistance(nextPacmanLocation, nearestGhostPosition) )

        return legal[nearestGhostDistance.index(min(nearestGhostDistance))]

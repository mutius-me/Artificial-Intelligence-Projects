...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            observationProb = self.getObservationProb(observation, 
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for key in newPosDist.keys():
                discreteDistribution[key] += self.beliefs[oldPos] * newPosDist[key]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            particle =  self.legalPositions[i % numberOfPositions]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particleLocation in self.particles:
            # Update distribution with new observation probability
            discreteDistribution[particleLocation] += self.getObservationProb(observation, 
                                                                              gameState.getPacmanPosition(), 
                                                                              particleLocation, 
                                                                              self.getJailPosition())

        # Handle case when all probabilities sum to zero
        if discreteDistribution.total() == 0:
            self.initializeUniformly(gameState)

        # Normalize and resample
        else:
            self.beliefs = discreteDistribution.normalize()
            for i in range(self.numParticles):
                resample = discreteDistribution.sample()
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            if self.particles[i] not in dict:
                positionDistribution = self.getPositionDistribution(gameState, self.particles[i])
                dict[self.particles[i]] = positionDistribution
                self.particles[i] = positionDistribution.sample()
            else:
